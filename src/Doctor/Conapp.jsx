...
function Conapp() {
  ...
  const getapp = async () => {
    try {
      ...
      if (response.data.msg === "Success") {
        const confirmed = response.data.value.filter(d => d.status === "confirmed");
  ...
  const cancelapp = async (id) => {
    try {
      const response = await axios.put(`/app/${id}`, { status: "cancelled" });
      if (response.data.msg === "Success") {
  ...
  const completedapp = async (id) => {
    try {
      const response = await API.put(`/app/${id}`, { status: "completed" });
      if (response.data.msg === "Success") {
  ...
  return (
    <>
      ...
      <div className="container my-5">
        <h3 className="text-center text-info mb-4">Confirmed Appointments</h3>
            ...
            <tbody>
              {appointments.length > 0 ? appointments.map((app, i) => (
                <tr key={app._id}>
                  ...
                  <td>
                    <button className="btn btn-success btn-sm" onClick={() => completedapp(app._id)}>
                ...
                <tr>
                  <td colSpan="8" className="text-muted py-3">No Confirmed Appointments 😕</td>
                </tr>
              )}
            </tbody>
          </table>
