...
function Penapp() {
  ...
  const getapp = async () => {
    try {
      const response = await API.get(`/app/d/${localStorage.getItem('doctor')}`);
      if (response.data.msg === "Success") {
        const pending = response.data.value.filter(d => d.status === "pending");
  ...
  return (
    <>
      ...
      <div className="container my-5">
        <h3 className="text-center text-info mb-4">Pending Appointments</h3>
            ...
            <tbody>
              {appointments.length > 0 ? appointments.map((app, i) => (
                <tr key={app._id}>
                  ...
                  <td>
                    <button className="btn btn-success btn-sm" onClick={() => acceptapp(app._id)}>
                ...
                <tr>
                  <td colSpan="8" className="text-muted py-3">No Pending Appointments 😕</td>
                </tr>
              )}
            </tbody>
          </table>
